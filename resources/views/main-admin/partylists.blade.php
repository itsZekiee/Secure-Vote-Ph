        ...
        <div x-show="showExportModal"
            ...
            <div class="flex items-center justify-center min-h-screen px-4 py-6 text-center">
                ...
                <div x-show="showExportModal"
                     ...
                     class="inline-block w-full max-w-2xl overflow-hidden text-left transition-all transform bg-white rounded-3xl shadow-2xl border border-slate-100">
                    ...
                    <div class="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h3 class="text-lg font-bold text-slate-900">Export Party Lists</h3>
                                <p class="text-xs text-slate-500 font-medium">Select party lists to include in the export</p>
                            </div>
                        </div>
                    ...
                    <div class="p-8">
                        <div class="flex items-center justify-between mb-4">
                            <label class="flex items-center space-x-2 cursor-pointer group">
                                ...
                                <span class="text-xs font-bold text-slate-600 group-hover:text-indigo-600 transition-colors uppercase tracking-widest">Select All</span>
                            </label>
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest" x-text="selectedPartylistsForExport.length + ' selected'"></span>
                        </div>

                        <div class="max-h-[300px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                            <template x-for="partylist in allPartylists" :key="partylist.id">
                                <div class="flex items-center justify-between p-3 rounded-xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/30 transition-all cursor-pointer group"
                                     @click="togglePartylistSelection(partylist.id)">
                                    <div class="flex items-center space-x-3">
                                        <input type="checkbox"
                                               :checked="selectedPartylistsForExport.includes(partylist.id)"
                                               class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 pointer-events-none">
                                        <div>
                                            <p class="text-sm font-bold text-slate-700" x-text="partylist.name"></p>
                                            <p class="text-[10px] text-slate-400 font-medium" x-text="partylist.acronym || 'No Acronym'"></p>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest" x-text="partylist.organization?.name || 'No Org'"></p>
                                    </div>
                                </div>
                            </template>
                        </div>
                    ...
                    <div class="px-8 py-6 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row-reverse gap-3">
                        <button type="button"
                                @click="exportSelected"
                                class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all">
                            <i class="ri-download-2-line mr-2"></i>
                            Download Export
                        </button>
                        <button type="button"
                                @click="showExportModal = false"
                                class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-2.5 bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-xl hover:bg-slate-50 active:scale-95 transition-all">
                            Cancel
                        </button>
                    </div>
                </div>
                ...
                <div x-show="showExportModal"
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button type="button"
                                ...
                                class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50">
                            Download CSV
                        </button>
                        <button type="button"
                                @click="showExportModal = false"
