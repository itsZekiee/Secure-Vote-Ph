...
@section('content')
    <div x-data="{
        ...
        {{-- Modal --}}
        <div x-show="showModal" x-cloak class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div x-show="showModal" class="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden"
                 x-transition:enter="transition ease-out duration-300"
                 x-transition:enter-start="opacity-0 scale-95"
                 x-transition:enter-end="opacity-100 scale-100">
                <div class="p-8 text-center" :class="modalType === 'success' ? 'bg-gradient-to-r from-emerald-50 to-teal-50' : 'bg-gradient-to-r from-red-50 to-pink-50'">
                    <div class="mx-auto w-24 h-24 rounded-full flex items-center justify-center mb-6" :class="modalType === 'success' ? 'bg-emerald-100' : 'bg-red-100'">
                ...
                <div class="p-6 bg-gray-50 flex justify-center space-x-4">
                    <button @click="closeModal()" class="px-6 py-3 rounded-xl bg-gray-700 text-white font-medium hover:bg-gray-800 transition-colors">Close</button>
                    <a x-show="modalType === 'success'" href="{{ route('admin.partylists.index') }}" class="px-6 py-3 rounded-xl bg-white border-2 border-gray-200 text-gray-700 font-medium hover:bg-gray-50 transition-colors">View All</a>
                </div>
            </div>
        </div>
        ...
        <header class="sticky top-0 z-40 bg-white/90 backdrop-blur-xl border-b border-gray-200/50 shadow-sm">
            <div class="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
                <nav class="flex items-center text-sm space-x-3">
                    <a href="{{ route('admin.dashboard') }}" class="text-gray-600 hover:text-blue-600 transition-colors"><i class="ri-home-4-line text-lg"></i></a>
                    <span class="text-gray-400">/</span>
                    <a href="{{ route('admin.partylists.index') }}" class="text-gray-600 hover:text-blue-600 transition-colors font-medium">Party Lists</a>
                    <span class="text-gray-400">/</span>
                    <span class="font-bold text-gray-900">Create</span>
                </nav>

                <div class="flex items-center space-x-3">
                    <button @click="loadDraft()" class="px-4 py-2.5 text-sm border-2 border-gray-200 rounded-xl font-medium hover:border-blue-300 hover:text-blue-600 transition-all">
                        <i class="ri-download-line mr-2"></i>Load Draft
                    </button>
                    <button @click="saveDraft()" class="px-4 py-2.5 text-sm bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-medium hover:from-blue-700 hover:to-indigo-700 transition-all shadow-md">
        ...
        <section class="max-w-7xl mx-auto px-6 py-10">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-6">
                    <div class="w-16 h-16 bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl flex items-center justify-center text-white text-3xl shadow-lg">
                ...
                <div class="flex items-center space-x-3">
                    <button @click="resetForm()" class="px-5 py-3 border-2 border-gray-200 rounded-xl font-medium hover:border-red-300 hover:text-red-600 transition-all">
                        <i class="ri-refresh-line mr-2"></i>Reset
                    </button>
                    <button @click="showPreview = !showPreview" class="px-5 py-3 bg-gray-100 border-2 border-gray-200 rounded-xl font-medium hover:bg-gray-150 transition-all">
        ...
        <main class="max-w-7xl mx-auto px-6 pb-12">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
                    ...
                    <div x-show="Object.keys(errors).length" class="mb-6 p-4 rounded-xl border-l-4 border-red-500 bg-red-50">
                                ...
                                <div>
                                    ...
                                    <div
                                        ...
                                        <div x-show="logoPreview" class="relative">
                                            ...
                                            <button type="button" @click="formData.logo = null; logoPreview = null; formData.logoName = ''"
                                ...
                                <div>
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div class="text-center">
                                            <span class="text-sm font-mono px-3 py-1 bg-gray-100 rounded" x-text="formData.color"></span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div x-show="currentStep === 2" class="space-y-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="p-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl border border-green-100" x-show="formData.platform">
                                <h4 class="font-bold text-gray-900 mb-4 flex items-center">
                            ...
                            <div class="p-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl border border-purple-100">
                                <h4 class="font-bold text-gray-900 mb-4 flex items-center">
                                ...
                                <div class="flex items-center space-x-6">
                                    <div class="flex-shrink-0">
                                        ...
                                        <div x-show="!logoPreview" class="w-16 h-16 rounded-2xl flex items-center justify-center text-white text-2xl font-bold shadow-md"
                                             :style="'background-color:' + formData.color" x-text="(formData.name || 'P').charAt(0)">
                                        </div>
                                    </div>
                                    <div>
                                        <div class="text-sm">
                                        ...
                                        <div class="text-xs text-gray-500 mt-1" x-text="formData.logoName ? formData.logoName : 'No logo uploaded'"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-2 border-gray-100 rounded-xl p-4 bg-gradient-to-br from-gray-50 to-white">
                            <div class="flex items-center space-x-4">
                                <div class="flex-shrink-0">
                                    ...
                                    <div x-show="!logoPreview" class="w-14 h-14 rounded-xl flex items-center justify-center text-white font-bold shadow-md"
                                         :style="'background-color:' + formData.color" x-text="(formData.name || 'P').charAt(0)">
                                    </div>
                                </div>
