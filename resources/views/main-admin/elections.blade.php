...
@section('content')
    ...
    function validatePositionsData(positions) {
        const posSet = new Set();

        for (const pos of positions) {
            const p = normalize(pos.name);
            if (posSet.has(p)) {
                alert(`Duplicate position: ${pos.name}`);
                return false;
            }
            posSet.add(p);
            ...
            for (const c of pos.candidates) {
                const n = normalize(c);
                if (candSet.has(n)) {
                    alert(`Duplicate candidate "${c}" in ${pos.name}`);
                    return false;
                }
                candSet.add(n);
            }
                                                    ...
                                                    <div class="bg-white border-2 border-gray-200 rounded-2xl p-8 shadow-lg hover:shadow-xl transition-shadow">
                                                        <div class="flex items-center gap-4 mb-6">
                                                            ...
                                                            <div class="flex-1">
                                                                ...
                                                                <input type="text"
                                                                    ...
                                                                    :name="'positions[' + pIndex + '][name]'"
                                                                    @blur="
                                                                            position.name = toTitleCase(position.name);

                                                                            const current = normalize(position.name);
                                                                            if (!current) return;

                                                                            const duplicate = parentData.positions.some((p, idx) =>
                                                                                idx !== pIndex && normalize(p.name) === current
                                                                            );
                                                                    ...
                                                                    class="block w-full rounded-xl border-gray-300 shadow-sm
                                                                            ...
                                                                            px-5 py-4 text-base transition-all"
                                                        ...
                                                        <div class="space-y-4 pl-16">
                                                            ...
                                                            <template x-for="(candidate, cIndex) in position.candidates" :key="cIndex">
                                                                <div class="flex items-center gap-3">
                                                                        <input type="text"
                                                                            x-model="position.candidates[cIndex]"
                                                                            :name="'positions[' + pIndex + '][candidates][]'"
                                                                            @blur="
                                                                                    position.candidates[cIndex] = toTitleCase(position.candidates[cIndex]);

                                                                                    const current = normalize(position.candidates[cIndex]);
                                                                                    if (!current) return;

                                                                                    // Check duplicate across all positions
                                                                                    const duplicate = parentData.positions.some((p, pi) =>
                                                                                        p.candidates.some((c, ci) =>
                                                                                            (pi !== pIndex || ci !== cIndex) && normalize(c) === current
                                                                                        )
                                                                                    );

                                                                                    if (duplicate) {
                                                                                        alert('This candidate already exists in another position.');
                                                                                        position.candidates[cIndex] = '';
                                                                                        $event.target.focus();
                                                                                    }
                                                                            "
                                                                            class="flex-1 rounded-xl border-gray-300 shadow-sm
                                                                                    focus:ring-2 focus:ring-indigo-500 focus:border-transparent
                                                                                    px-5 py-3 text-base transition-all"
                                    ...
                                    <div class="space-y-8">
                                        ...
                                        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-2xl p-6">
                                            <label class="flex items-start justify-between cursor-pointer gap-4">
                                                ...
                                                <div class="relative flex-shrink-0">
                                                    <input type="checkbox" x-model="enableGeo" name="enable_geo_location" class="sr-only peer" id="geoToggle" value="1">
                                        ...
                                        <div class="bg-gradient-to-r from-amber-50 to-orange-50 border-2 border-amber-200 rounded-2xl p-6">
                                            <label class="flex items-start justify-between cursor-pointer gap-4">
                                                ...
                                                <div class="relative flex-shrink-0">
                                                    <input type="checkbox" x-model="formData.enable_geo_registration" name="enable_geo_registration" class="sr-only peer" id="geoRegToggle" value="1">
