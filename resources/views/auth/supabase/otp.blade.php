...
<body class="bg-[#f8fafc] min-h-screen flex items-center justify-center p-4">
    <div class="max-w-[480px] w-full">
        ...
        <div class="text-center mb-10">
            ...
            <p class="text-[11px] text-slate-500 font-bold tracking-widest uppercase mt-1">Protecting your democratic voice</p>
        </div>
            ...
            <div class="p-8 sm:p-12">
                <div class="mb-8">
                    ...
                    <p class="text-sm text-slate-500 font-medium mt-2 leading-relaxed">
                        We've sent a 6-digit verification code to <span class="text-indigo-600 font-bold">{{ session('otp_email') }}</span>.
                        Please enter it below to continue.
                    </p>
                </div>

                @if($errors->any())
                    <div class="mb-6 p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-start gap-3 text-rose-600">
                ...
                <form method="POST" action="{{ $verify_route ?? route('otp.verify') }}" class="space-y-8">
                    ...
                    <div class="space-y-4">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Verification Code</label>
                        <div class="relative group">
                            <i class="ri-key-2-line absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600 transition-colors text-xl"></i>
                            <input type="text"
                                   name="token"
                                   placeholder="000000"
                                   maxlength="8"
                ...
                <div class="mt-10 pt-8 border-t border-slate-50 text-center">
                    <p class="text-[11px] text-slate-400 font-bold uppercase tracking-widest mb-4">Didn't receive the code?</p>
                    <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                        <button type="button" class="text-indigo-600 font-black text-[10px] uppercase tracking-widest hover:text-indigo-700 transition-colors">
                            Resend Code
                        </button>
                        <span class="hidden sm:block w-1.5 h-1.5 rounded-full bg-slate-200"></span>
                        <a href="{{ $back_route ?? route('home') }}" class="text-slate-500 font-black text-[10px] uppercase tracking-widest hover:text-slate-700 transition-colors">
                            Back to Login
                        </a>
                    </div>
                </div>
